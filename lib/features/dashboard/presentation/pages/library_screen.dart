            ...
            child: Container(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  // Remove ✕ button (long-press to reveal)
                  if (_showRemove)
                    Positioned(
                      top: 8,
                      right: 8,
...
class _LibraryScreenState extends ConsumerState<LibraryScreen> {
  ...
  List<dynamic> _applyFilters(List<dynamic> library) {
    return library.where((m) {
      final q = _query.toLowerCase();
      final matchSearch =
          q.isEmpty ||
          (m.title as String).toLowerCase().contains(q) ||
          (m.author as String).toLowerCase().contains(q);
      final matchFilter =
          _filter == _Filter.all ||
          (m.status as String).toLowerCase() == _filter.name;
      return matchSearch && matchFilter;
    }).toList()..sort((a, b) {
      switch (_sort) {
        case _Sort.title:
        ...
        case _Sort.addedAt:
          return DateTime.parse(
            b.addedAt as String,
          ).compareTo(DateTime.parse(a.addedAt as String));
      }
    });
  }

  SliverGrid _buildGrid(List<dynamic> items) {
    return SliverGrid(
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 3,
        childAspectRatio: 0.52,
      ...
      delegate: SliverChildBuilderDelegate(
        (context, i) => _GridCard(
          ...
          onTap: () {}, // navigate to detail
          onRemove: () =>
              setState(() => _removeId = items[i].mangaId as String),
        ),
        childCount: items.length,
      ),
    );
  ...
  SliverList _buildList(List<dynamic> items) {
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        (context, i) => Padding(
          padding: const EdgeInsets.only(bottom: 10),
...
class _HeroStats extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final avgRating = library.isEmpty
        ? 0.0
        : library.fold<double>(0, (s, m) => s + (m.rating as num)) /
              library.length;
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    if ((m.rating as num?) != null && m.rating > 0)
                      Text(
...
class _ListRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final m = item;
    final genres = (m.genre as List?)?.take(2).toList() ?? [];
...
class _RemoveModal extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final m = library.cast<dynamic>().firstWhere(
      (x) => x?.mangaId == mangaId || x?.id == mangaId,
      orElse: () => null,
    );
